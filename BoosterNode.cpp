...
//sprintf(myMqttMsg, multiByteResponse, "\"SVW4\"", lnAddress, 0x46, ((nodeNr+1) << 8) + 0x28, scValBuf[0]);

uint8_t initPwrPin[6] = {12,9,11,4,5,7};
uint8_t initARPin[6] = {13,10,0,0,0,0};

//char myMqttMsg[80];
uint16_t scValBuf[17];
...
//    Serial.println(bCfg.sensePin);
    sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", lnAddress, 0x46, ((nodeNr+1) << 8) + 0x00, (bCfg.ctrlPin << 4) + (bCfg.sensePin-A0), bCfg.reversePin, sensVal >> 8, sensVal & 0x00FF);
...
//    Serial.println(bCfg.autoReverseMode);
    sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", lnAddress, 0x46, ((nodeNr+1) << 8) + 0x04, (bCfg.autoReverseMode << 2) + (bCfg.autoResetMode & 0x03), bCfg.fuseMode, bCfg.currNominal >> 8, bCfg.currNominal & 0x00FF);
    Serial.print(myMqttMsg);
    return true;
  }
  ...
  {
    reportFlags &= ~0x04;
    sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", lnAddress, 0x46, ((nodeNr+1) << 8) + 0x08, bCfg.actSetOn.trigDef, (bCfg.actSetOn.devAddr >> 8),(bCfg.actSetOn.devAddr & 0x00FF),0);
    Serial.print(myMqttMsg);
    return true;
  }
  ...
  {
    reportFlags &= ~0x08;
    sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", lnAddress, 0x46, ((nodeNr+1) << 8) + 0x0C, bCfg.actSetOff.trigDef, (bCfg.actSetOff.devAddr >> 8),(bCfg.actSetOff.devAddr & 0x00FF),0);
    Serial.print(myMqttMsg);
    return true;
  }
  ...
  {
    reportFlags &= ~0x10;
    sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", lnAddress, 0x46, ((nodeNr+1) << 8) + 0x10, bCfg.actResetNode.trigDef, (bCfg.actResetNode.devAddr >> 8),(bCfg.actResetNode.devAddr & 0x00FF),0);
    Serial.print(myMqttMsg);
    return true;
  }
  ...
  {
    reportFlags &= ~0x20;
    sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", lnAddress, 0x46, ((nodeNr+1) << 8) + 0x14, bCfg.actARPolarityPos.trigDef, (bCfg.actARPolarityPos.devAddr >> 8),(bCfg.actARPolarityPos.devAddr & 0x00FF),0);
    Serial.print(myMqttMsg);
    return true;
  }
  ...
  {
    reportFlags &= ~0x40;
    sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", lnAddress, 0x46, ((nodeNr+1) << 8) + 0x18, bCfg.actARPolarityNeg.trigDef, (bCfg.actARPolarityNeg.devAddr >> 8),(bCfg.actARPolarityNeg.devAddr & 0x00FF),0);
    Serial.print(myMqttMsg);
    return true;
  }
  ...
  {
    reportFlags &= ~0x80;
    sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", lnAddress, 0x46, ((nodeNr+1) << 8) + 0x1C, bCfg.repShortCircuit.trigDef, (bCfg.repShortCircuit.devAddr >> 8),(bCfg.repShortCircuit.devAddr & 0x00FF),0);
    Serial.print(myMqttMsg);
    return true;
  }
  ...
  {
    reportFlags &= ~0x100;
    sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", lnAddress, 0x46, ((nodeNr+1) << 8) + 0x20, bCfg.repOverload.trigDef, (bCfg.repOverload.devAddr >> 8),(bCfg.repOverload.devAddr & 0x00FF),0);
    Serial.print(myMqttMsg);
    return true;
  }
  if (reportFlags & 0x200) //Short circuit current average
  {
    reportFlags &= ~0x200;
    sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", lnAddress, 0x46, ((nodeNr+1) << 8) + 0x24, bCfg.scCurr >> 8, bCfg.scCurr & 0x00FF,0, 0);
    Serial.print(myMqttMsg);
    return true;
  }
  if (reportFlags & 0x400) //Short circuit current measurement data
  {
    reportFlags &= ~0x400;
    sprintf(myMqttMsg, multiByteResponse, "\"\SVW4\"", lnAddress, 0x46, ((nodeNr+1) << 8) + 0x28, scValBuf[0],scValBuf[5],scValBuf[11],scValBuf[16]);
    Serial.print(myMqttMsg);
    return true;
  }
...
//  Serial.println(*rdPtr);
//  Serial.println();
//  Serial.print("{\"Mod Check\":");
//  Serial.print(readData.checkByte);
//  Serial.print(",\"Sum\":");
//  Serial.print(chkSum);
//  Serial.println("}");
  if ((chkSum != readData.checkByte) || (chkSum == 0xFF) || (chkSum == 0x00))
  {
    Serial.println("Init Node");
    readData.ctrlPin = initPwrPin[nodeNr];;
    readData.sensePin = A0 + nodeNr; //analog pins starting with 14
    readData.reversePin = initARPin[nodeNr];
    readData.scCurr = 0;
    readData.senseFactor = 12.2; //Current Factor: 8500 A/A, Rsense = 5kOhm Umax at 8.5 Amps = 5V = Analog Value 1023 Sense Factor = 8500 / 1023 = 8.31
    readData.currNominal = 3000; //[mA] set trip current, must be smaller than achievable short circuit current. 80% of this is nominal current (infinite RMS load) if higher, warmup is calculated
    readData.fuseMode = 30; //10,20,30,..,100
    readData.autoResetMode = 1; //0: manual only 1: limited 2: full autoreset
...
void Booster::processTimerSCDetect()
{
  if (bOps.scCtr < scTestSamples)
  {
    ...
    {
      uint8_t bufPos = bOps.scCtr>>4;
      if ((bOps.scCtr & 0x000F) == 0)
      {
        scValBuf[bufPos] = analogRead(bCfg.sensePin);
      }
      bOps.scVal += scValBuf[bufPos];
//      tripLine(false);
      bOps.scCtr++;
    }
  }
  else
  {
    ...
    reportFlags |= 0x400; //value array
    reportFlags |= 0x200; //average value
    bOps.extStat = extStop; 
  } 
}
