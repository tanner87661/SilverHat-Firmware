...
//  Serial.println("}");
  ...
  {
    Serial.println("Init Dev");
    readData.numMods = 1;
    readData.serNr = UniqueID[0]; //random(255);
...
*/
      sigStatus = hasSig;
//      digitalWrite(LED_BUILTIN, hasSig);
...
bool BoosterGroup::processReportRequest()
{
  if ((millis() - lastReportTime) > reportInterval) //rollover safe
  {
    ...
    if (reportQueue.hasRequest())
    {
      ...
      if (thisReq & 0x0100)
        sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", bConfig.lnAddress, 0x46, thisReq & 0xFF, getMemoryPos(thisReq & 0x00FF), getMemoryPos((thisReq & 0x00FF) + 1), getMemoryPos((thisReq & 0x00FF) + 2), getMemoryPos((thisReq & 0x00FF) + 3));
      else
        sprintf(myMqttMsg, multiByteResponse, "\"SVR1\"", bConfig.lnAddress, 0x42, thisReq & 0xFF, getMemoryPos(thisReq & 0x00FF),0,0,0);
      Serial.print(myMqttMsg);
      return true;
    }
//    if (reportFlags > 0)
//    {
//    sprintf(myMqttMsg, "{\"F\":%i}\n", reportFlags);
//    Serial.print(myMqttMsg);
//    }
    ...
    {
      reportFlags &= ~0x01;
      sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", bConfig.lnAddress, 0x47, 0x00, manufID, devID, prodID, bConfig.serNr);
      Serial.print(myMqttMsg);
      return true;
    }
    if (reportFlags & 0x02) //
    {
      reportFlags &= ~0x02;
      sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", bConfig.lnAddress, 0x48, 0x00, manufID, devID, prodID, bConfig.serNr);
      Serial.print(myMqttMsg);
      return true;
    }
    ...
    {
      reportFlags &= ~0x04;
      sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", bConfig.lnAddress, 0x46, 0x00, bConfig.numMods, bConfig.useLN, ((bConfig.lnAddress & 0xFF00)>>8), (bConfig.lnAddress & 0x00FF));
      Serial.print(myMqttMsg);
      return true;
    }
    ...
    {
      reportFlags &= ~0x08;
      sprintf(myMqttMsg, multiByteResponse, "\"SVR1\"", bConfig.lnAddress, 0x42, 0x04, bConfig.devMode,0,0,0);
      Serial.print(myMqttMsg);
      return true;
    } 
    ...
    {
      reportFlags &= ~0x10;
      sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", bConfig.lnAddress, 0x46, 0x08, bConfig.globalOn.trigDef, (bConfig.globalOn.devAddr >> 8),(bConfig.globalOn.devAddr & 0x00FF),0);
      Serial.print(myMqttMsg);
      return true;
    }
    ...
    {
      reportFlags &= ~0x20;
      sprintf(myMqttMsg, multiByteResponse, "\"SVR4\"", bConfig.lnAddress, 0x46, 0x0C, bConfig.globalOff.trigDef, (bConfig.globalOff.devAddr >> 8),(bConfig.globalOff.devAddr & 0x00FF),0);
      Serial.print(myMqttMsg);
      return true;
    }
    ...
    {
      reportFlags &= ~0x0100;
      sprintf(myMqttMsg, multiByteResponse, "\"SVR1\"", bConfig.lnAddress, 0x42, 0xF0, sigStatus,0,0,0);
      Serial.print(myMqttMsg);
      return true;
    }
    ...
    {
      reportFlags &= ~0x0200;
      sprintf(myMqttMsg, multiByteResponse, "\"SVR1\"", bConfig.lnAddress, 0x42, 0xF1, pwrStatus,0,0,0);
      Serial.print(myMqttMsg);
      return true;
    }
    for (uint8_t i = 0; i < bConfig.numMods; i++)
